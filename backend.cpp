...
void saveSong(const Song& s) {
    ...
    if (!file.is_open()) return;
    file << s.id << " " << s.name << "|" << s.artist << "|" << s.genre << "|" << s.rating << "\n";
    file.close();
}
...
public:
    ...
    void run() override {
        ...
        for (const auto& song : allSongs) {
            ...
            std::transform(tempGenre.begin(), tempGenre.end(), tempGenre.begin(), ::tolower);
            if (tempGenre == lowerTarget) results.push_back(song);
        }
...
public:
    ...
    void run() override {
        ...
        for (const auto& s : allSongs) {
            ...
            if (s.genre == target.genre) score -= 0.5f; // bonus for same genre
